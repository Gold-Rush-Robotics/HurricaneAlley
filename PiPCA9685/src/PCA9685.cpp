...
PCA9685::PCA9685(int address) {
  i2c_dev = std::make_unique<I2CPeripheral>(address);
...
void PCA9685::set_pwm_pow(const int channel, const double power){
    auto full = 1/frequency;
    set_pwm_ms(channel, full*power);
}
